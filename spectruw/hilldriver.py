    ...
    def _continue():
        ...
        # Look to see if non-local coefficients are present, and format it
            ...
            def _select_mu(widget):
                ...
                intvar = int(w_spectrum.getvar(widget.cget('variable')))
                itemindex = numpy.where(abs(MU - m) < numpy.finfo(numpy.single).eps)
                if (intvar==1):
                    spectrumplot.plot(real(A[0][itemindex[0][0]]),imag(A[0][itemindex[0][0]]),'yo',picker=5)
                else:
                    spectrumplot.plot(real(A[0][itemindex[0][0]]),imag(A[0][itemindex[0][0]]),'bo',picker=5)
                plot.canvas.draw()

            canvas = Canvas(w_spectrum, borderwidth=0, background="#ffffff")
            frame = Frame(canvas, background="#ffffff")
            vsb = Scrollbar(w_spectrum, orient="vertical", command=canvas.yview)
            ...
            Tk.Label(master=frame, text='Mu Values:').pack(side=Tk.TOP)
            
            for muval in MU:
                mu_selected = Tk.IntVar()
                cb=Tk.Checkbutton(master=frame, text=str(muval), justify=LEFT, variable=mu_selected, command=_select_mu)
                cb.pack(side=Tk.TOP,fill=Tk.BOTH)
                cb.configure(command=lambda widget=cb:_select_mu(widget))



            plot = plt.figure()
            canvas2 = FigureCanvasTkAgg(plot, master=w_spectrum)
            spectrumplot = plot.add_subplot(211)
            spectrumplot.set_title('Click on eigenvalue to plot eigenfunction:')
            line, = spectrumplot.plot(spectrumx,spectrumy, 'bo', picker=5) #5 points tolerance

            # eigenfunction plot
            efcnplot = plot.add_subplot(212)
            efcnplot.set_title('Eigenfunction')
            ...
            toolbar = NavigationToolbar2TkAgg( canvas2, w_spectrum )
            toolbar.update()
